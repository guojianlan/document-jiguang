...
URL Source: https://idoubi.ai/blog/my-vibe-coding-projects
...
![Image 1](https://cdn.shipany.ai/imgs/48bcf4cfa881f2baa5a1c244f653aa7a.png)
...
2.   不敢在自己电脑跑，担心隐私和安全风险

3.   不懂技术，搞不了虚拟机，折腾不来云服务器

4.   希望长时间待机，但没预算，不想买 Mac Mini

5.   不想学怎么配模型、Skills，只想开箱即用
...
![Image 2](https://cdn.shipany.ai/imgs/c789a6bd6deaee57c7a4aa2c750fa5b4.png)
...
![Image 3](https://cdn.shipany.ai/imgs/f9201e1a1a0d0cd709327ee8fbd89c59.png)

ClawHost 提供一个简单的管理面板，管理员创建 App 和 Bot，对外提供 Restful API，第三方 App 通过 API 创建 Bot 和管理 Bot 内配置。

![Image 4](https://cdn.shipany.ai/imgs/fbb738f8b1c8ab3c0f08ccca0c29013f.png)
...
![Image 5](https://cdn.shipany.ai/imgs/816cec77ac7c087da54e21ff4979cb48.png)
...
![Image 6](https://cdn.shipany.ai/imgs/b66121c9286feebb3b02fefe47f3b89e.png)
...
![Image 7](https://cdn.shipany.ai/imgs/6ca82db17c9ed62e1b787bb34d006657.png)
...
1.   OpenClaw 是单租户架构，我需要为每个用户单独用一个 Pod 隔离部署，挂载独立的存储空间

2.   OpenClaw 需要至少 1G 的运行内存，在 K8S 部署，每个 Pod 的 memory_limit 不能低于 4G，不然很容易 OOM

3.   OpenClaw 针对一些复杂类任务（比如：写代码），会开独立的子进程去执行，进程开销很大。

4.   OpenClaw 启动依赖网关初始化、插件加载等多个串行步骤，从启动到可用需要 15-30 秒，没办法做到秒级启动。如果要保证用户云端的 Bot 一直在线，Pod 需要常驻，对于不经常使用的 Bot，是一种资源浪费
...
1.   使用 Go 实现，单二进制分发（5MB），运行零依赖，秒级启动

2.   运行时内存占用 ~20M，约为 OpenClaw 内存占用的 1/7

3.   约 3000 行代码实现 OpenClaw 的核心功能，二次开发更简单

4.   以 RPC 进程通信的方式运行插件，隔离风险，插件挂了不影响主进程，解决 OpenClaw Gateway 容易挂的问题

5.   引入数据库实现持久化存储，用户会话隔离，天然适用云原生多租户部署场景

6.   可视化引导安装，上手门槛更低。管理后台 Web UI 更清晰、操作更友好

![Image 8](https://cdn.shipany.ai/imgs/0b1e09bf1995bf486de9528e3a09663c.png)

除了架构方面的优化和对云端多租户场景的适配，FastClaw 选择尽可能兼容 OpenClaw 的协议和生态：

1.   fastclaw.json 配置文件，采用跟 openclaw.json 一样的数据结构
2.   支持 OpenClaw 的 plugins，支持 ClawHub 的 skills 等
3.   跟 OpenClaw 类似的方式实现 memory、context、tools 等核心功能
...
![Image 9](https://cdn.shipany.ai/imgs/db988986caf1d2077edda95a5de9bca0.png)
...
![Image 10](https://cdn.shipany.ai/imgs/f7653669aed7bb0c70477bd93d94ef69.png)
...
1.   做转换器。把互联网上的 OpenAPI Schema / 自定义的脚本、Pipeline / 网站 / 桌面 App 等，一键转换成 CLI 工具
2.   做扩展坞。输入是 CLI，输出是 MCP / Skills，让不同类型的 Agent 都能理解工具，在合适的时机自动发现并调用工具
3.   做工具箱。把常用的工具收集起来，放到工具箱，让 Agent 可以搜索、安装、调用工具
...
![Image 11](https://cdn.shipany.ai/imgs/6514a8cab847d95b7b2690baca4764cf.png)
...
![Image 12](https://cdn.shipany.ai/imgs/e4e3f3caf7443f314580ec664916d85b.png)
...
![Image 13](https://cdn.shipany.ai/imgs/a541b7c5ed6905e8d7d5f258bd15640e.png)
...
![Image 14](https://cdn.shipany.ai/imgs/aa271d1a071da5c2a8c919dc98d4aac3.png)
...
![Image 15](https://cdn.shipany.ai/imgs/766d1753f2fe1435a4a792ce558b8fa9.png)
...
![Image 16](https://cdn.shipany.ai/imgs/160a23940d25336f8d47f79f077f8a62.png)
...
![Image 17](https://cdn.shipany.ai/imgs/57b50183f672112b8b3f8a0d2761f82e.png)
...
![Image 18](https://cdn.shipany.ai/imgs/e55efdfd6809bd3d0618761ec2fccaaf.png)

CCOnline 目前还在技术实验阶段，我没想明白纯云端的 Claude Code 有什么价值，目标群体和使用场景分别是什么。

用一个表格整理了以上提到的几个项目👇

![Image 19](https://cdn.shipany.ai/imgs/d2a72a6e29ce54b0b895dbee0ac84203.png)
