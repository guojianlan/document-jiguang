...
related_topics: [claude-code, codex, ai-workflow, vibe-coding-path, ai-agent]
status: active
...
OpenSpec 不该作为 Vibecoding 入门工具——但它是 [[vibe-coding-path|AI Vibecoding]] **第 3 阶段（工作流升级）和第 5 阶段（质量与安全）的具体落地**。三条独立证据：
...
- **vs [[claude-code|Claude Code]]**：Claude Code 是 coding agent 工具产品，OpenSpec 是规格驱动的协作流程框架——叠加使用最佳：OpenSpec 写 change，Claude Code 跑实现
- **vs [[codex|Codex]]**：Codex 用 AGENTS.md + cloud sandbox 路线，OpenSpec 用 proposal/design/spec/tasks 工件路线；前者把约定写在仓库根，后者把约定写在每次 change
- **配 Superpowers**：OpenSpec 管"做什么"（业务行为，what），Superpowers 管"怎么做"（工程实现步骤，how）。两者通过文件系统 Markdown 文件 handshake，不依赖 API。但要注意**两个 spec 的区分**——OpenSpec 的 spec 是业务行为定义；Superpowers 的实现计划里也有 spec 描述（工程步骤）。Superpowers 的 spec reviewer 检查的是后者，不是前者。两层 spec 是否对齐由 brainstorming 阶段保证。详见 [[openspec-superpowers_workflow_digest]]
- **`/opsx:apply` 归属**：是 OpenSpec 自身命令；同时装 Superpowers 时会附带触发 TDD 等技能，但底层执行是 OpenSpec 的，不要错把 apply 归给 Superpowers
...
- OpenSpec 与 [[ai-workflow|AI 工作流]] 的关系分析
- OpenSpec 最小实践案例
- 面向团队的落地建议与误区清单
